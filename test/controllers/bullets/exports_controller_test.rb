...
module Bullets
  class ExportsControllerTest < ActionDispatch::IntegrationTest
    setup do
      @user = users(:one)
      sign_in_as @user
      @first = @user.bullets.create!(bulletable: Task.create!, body: 'First bullet')
      @second = @user.bullets.create!(bulletable: Note.create!, body: 'Second bullet')
    end
    ...
    test 'show downloads html for selected bullets' do
      ...
      assert_match(/attachment; filename="digibujo-export-\d{4}-\d{2}-\d{2}\.html"/,
