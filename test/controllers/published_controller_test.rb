...
class PublishedControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'index lists published bullets for current user' do
    ...
    published.publish!
    @user.bullets.create!(bulletable: Task.new(body: 'Private task'))
  ...
  test 'show renders published bullet without authentication' do
    bullet = @user.bullets.create!(bulletable: Note.new(body: 'Shared content'))
    bullet.publish!
  ...
  test 'show returns not found after unpublish' do
    bullet = @user.bullets.create!(bulletable: Note.new(body: 'Was public'))
    bullet.publish!
