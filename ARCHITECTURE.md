...
**Chat composer** ([`bullets/_composer`](app/views/bullets/_composer.html.erb), `chat-composer` Stimulus): a fixed dock mounted on daylog and collection pages (`bottom` clears the floating tabbar / iOS keyboard). Callers pass `bucket_id` and optionally `pops_on`. One Lexxy `note` editor serves every type; a hidden `bulletable_type` field is switched by the type picker (`BulletsHelper::COMPOSER_TYPES` — Note/Task/Event; Voice is reached through the mic button only) and the last pick is remembered in `localStorage`. Enter submits, Shift+Enter breaks the line, and neither sends while the editor is expanded or a Lexxy prompt menu is open. The row is single-line by default: a `ResizeObserver` adds `composer--multiline` once the editor grows past its blank height, which drops the controls below the field. A `+` expand control is always visible for Note (hidden for Task/Event); the composer mounts one Lexxy `note` editor (toolbar in the DOM). Compact puts Note-only `uploadFile` in the actions row beside the mic (proxying Lexxy’s hidden toolbar control). A second text line or an attachment latches `composer--multiline` and wraps the chrome; the Lexxy formatting toolbar appears only on expand, sticky above a content area that grows up to the safe viewport. No remount, no full-screen sheet. Expand stays hidden until multiline. Starting a voice take **swaps the compose row** for a matching pill shell (`composer--voice`): pause on the left, live waveform in the middle, send on the right; discard appears after the take is stopped.
